...
import { useApplicationsQuery, useListenerProfileFormLinkMutation } from "../../../services/listener"; // Import your API hook
 ...
 const [
  ...
  return (
    <div className="application-requests-main">
        ...
        {isLoading ? (
          ...
          data?.data?.users.map((request, index) => (
            <div className="table-body" key={index}>
              ...
              <div>
                <p className="heading-text">
                  <a href={`/dashboard/listener-management/listeners-profile-approvals-docs?id=${request?.id}`}>View</a>
                </p>
              </div>
              <div>
                <div className="actions">
                  <img   onClick={() => handleSendFormLink(request?.id,request.fullName)} src={right} alt={right} />{" "}
                  <img  onClick={() => haldleReject(request?.id)} src={cancel} alt={cancel} />{" "}
                </div>
              </div>
            </div>
          ))
      ...
      <RejectionModal show={showRejectModal} rejectedUser={rejectedUser} refetch={refetch}
